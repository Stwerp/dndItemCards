...
\usepackage[top=0.15in, bottom=0in, left=0.15in, right=0in]{geometry}
...
\def\bwcards{0}% %{0 -- color, 1 -- black and white (mostly)}
\newcommand{\printcolors}[2]{%
  \ifnum\bwcards=0 \expandafter#1\else \expandafter#2\fi
}
...
\begin{tikzpicture}
    ...
    \foreach \filename in \cardfiles {%
        ...
        \renewcommand{\cardcontinues}{0} % 0 or 1
        ...
        % load macros
        %\setinteriorstyle
        \def\interioroption{\printcolors{fill stretch image=\cardbg}{fill=white}}
        % Draw the card
        \node[minimum width=\cardwidth, minimum height=\cardheight, draw, rectangle, fill=white, scale=1] at  ({\j * (\cardwidth + 0.12in)}, {-\i * (\cardheight + 0.02in)}) {%
            \begin{itemcard}{\cardtitle}
                \begin{center}
                    ...
                    %\begin{tcolorbox}[width=\linewidth, boxrule=1.5pt, colframe=black,  boxsep=0pt, top=0pt, bottom=0pt, left=0pt, right=0pt, enhanced, borderline={0.5mm}{0mm}{black}, interior style={\printcolors{fill stretch image=\cardb}]
                    \begin{tcolorbox}[width=\linewidth, boxrule=1.5pt, colframe=black,  boxsep=0pt, top=0pt, bottom=0pt, left=0pt, right=0pt, enhanced, borderline={0.5mm}{0mm}{black}, interior style={\interioroption} ]
                ...
                \begin{lowerpart}
                    \carddescfront
                    \ifnum\cardcontinues=1%
                        \vfill \hfill \textit{continued on back...}%
                    \fi%
                \end{lowerpart}
           \end{itemcard}
        };
